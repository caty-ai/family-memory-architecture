...
| もの | 用途 | 補足 |
|---|---|---|
| Git / GitHub | 設計文書・Issue・レビューの台帳 | このリポジトリ自体の運用方法（README「いまできていること、できていないこと」参照） |
| Python 3 | 全スクリプトの実行環境 | 3.14 で実測。3.13 以前は未検証・3.9 では一部テスト不合格を確認。コアは標準ライブラリのみ。content-lint / injection-lint の2本だけ PyYAML が必要 |
